...
The homepage hero keeps the H1 “Your AI agent. Your infrastructure.”, a source-available qualifier, and the 55-second demo video with labels Ask · Browser · Take control · Saved file. The caption discloses that the footage is from an earlier host-native run. A seven-row You-control table follows. The US licence offer sits below the recorded-task proof, not in the hero. Keep setup requirements beside the entry point and detailed implementation/licence answers in the linked guides.

The homepage proof section and repository README use `email-inbox-redacted.png`, the actual 1440×900 inbox capture from the 8 September 2026 email organisation test. The About page and README also show the original completion capture, `email-organised-redacted.png`. Private inbox details, task narration and obsolete internal details are irreversibly blurred; other pixels, task durations and results are unchanged. The test took 51 min 6 s across two runs; the final run completed in 20 min 49 s. The optional-review screenshot `needs-you.png` and retained `task-running.png` asset are pre-rename captures from 7 September. Other historical screenshots are not deployed assets. The demo file is `website/video/bothearth-demo.mp4`.
