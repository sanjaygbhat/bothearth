...
| Browser cookies and local storage | Per-computer Docker profile volume (`modelbot-<id>-profile`); kept by ordinary computer destruction unless `--wipe-profile` is requested. Settings → Computers lists signed-in site domains and can Forget this computer's logins, which removes only that profile volume so the bot must sign in again; the agent-home volume stays. |
...
When the public contact form is enabled, it accepts enquiries without sign-in and forwards the submitted name, reply email, optional organisation, licence quantity, and message through Resend to the private support inbox. Reply addresses are unverified. That stateless form service keeps no enquiry database; temporary email hashes support rate limiting. Provider and mailbox retention apply separately. When the contact endpoint is not configured, the form cannot send a message. Check [the contact page](https://bothearth.com/contact/) for current availability.
