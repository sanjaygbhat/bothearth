...
The computer image includes the unmodified official CLI. Claude Code is listed in Settings; it is not yet verified live. Settings starts its native authentication inside the computer; tasks use its documented print/stream-JSON interface with stock native tools. See [Claude Code setup](CLAUDE-CODE.md).
...
Standalone adapters and local endpoints are a separate YAML path, not interchangeable with native Codex or Claude Code sign-in. Confirm model/tool-call compatibility and pricing yourself; the Chat Completions adapter does not support GPT-5.6 or newer. Local model servers are separately installed and licensed; model weights can have different terms from server software. See [configuration](CONFIG.md) and [extending BotHearth](EXTENDING.md).
...
Only automate accounts and data you are authorized to use. A successful login, public page, or approval in BotHearth does not override a website's terms, data rights, or anti-bot rules. A policy override is a local software setting, not legal permission. Optional review prompts for sends, uploads, deletes, payments and new sites are off by default; turn them on under Settings → Sensitive actions. Password, OTP, passkey and CAPTCHA still stop the task.

The receipt shows an estimate of tool use, not a bill: one estimated cent per BotHearth MCP computer-tool call by default. It does not fully count stock CLI commands or model requests. There is no BotHearth spend cap or call cap; a task runs until done, until you stop it, or until your model subscription’s usage limit is reached (then it pauses and can resume). Native shell/network tools also bypass MCP action checks. API estimates depend on configuration and may overshoot while a request is in flight. Set provider-side limits where available, and review the [security boundaries](../SECURITY.md) before relying on automation.
