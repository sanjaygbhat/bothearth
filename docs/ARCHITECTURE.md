...
Operator UI sessions and model MCP tokens have different authority. The MCP credential cannot grant operator approvals or watch human-control frames. Approvals bind the task, action, destination, control epoch and expiry. Normal public browsing proceeds without destination prompts; strict mode restricts destinations. Optional review prompts are off by default (`policy.gates: []`). Settings → Sensitive actions → **Ask before sensitive actions** writes the six optional classes, a subset, or clears the list, and can set API-adapter max tool calls and spend cap (0 = no limit); native Codex and Claude Code tasks have no BotHearth cap. An explicit `policy.gates` list in modelbot.yaml still asks. Password, OTP, passkey, CAPTCHA, payment-card entry and force-human categories always pause for Take control; they are not gates. Detected sends, uploads, deletes, checkout steps without a card field, and new-site form submits are optional MCP gates. Native shell/network operations bypass these action checks, and native CLI permission prompts are disabled inside the container.
...
Native usage is estimated from MCP calls, not every stock CLI command or model request. Standalone API estimates depend on reported usage and configured prices. There is no BotHearth spend cap or call cap. A Home native task runs until done, until you stop it, or until the model provider’s usage limit pauses it. The receipt is an estimate of tool use, not a bill. See [quickstart estimates](QUICKSTART.md#what-the-task-cost-means).
...
For full request validation, read [daemon/server.ts](https://github.com/sanjaygbhat/bothearth/blob/main/src/daemon/server.ts). This pre-release API is not a version-stability promise. Runtime readiness checks the actual Node version, runtime, stamped images, browser, and configured model connection; the home screen can open before task execution is ready. Home shows one runtime blocker card at a time. The ids are `node_version`, `docker_missing`, `docker_not_running`, `images_missing`, `images_stale`, `browser_unavailable`, `ai_not_connected`, and `ai_limit_reached`. There is no UI consumer of `/api/v1/routines`. Browser downloads sit in `/quarantine` tmpfs until an authenticated UI session with CSRF promotes them; there is no MCP promote tool. [Troubleshooting](TROUBLESHOOTING.md) maps the visible blockers to recovery actions.
